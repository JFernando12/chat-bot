...
class RecommendationResult:
    """Result object for car recommendations."""
    cars: List[Car]
    total_matches: int
...
class CarSearchServiceInterface(ABC):
    """Abstract interface for car search service."""
    
    @abstractmethod
    def search_by_text(self, query: str) -> List[Car]:
        """Search cars by natural language query."""
        pass
    
    @abstractmethod
    def fuzzy_search_make_model(self, make_model: str) -> List[Car]:
        """Fuzzy search for cars by make and model."""
        pass


class CarSearchService(CarSearchServiceInterface):
    """
